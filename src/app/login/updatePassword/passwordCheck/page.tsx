...
const PasswordCheckPage: React.FC = () => {
  ...
  const currentUpdatePasswordState = useRecoilValue(updatePasswordState);
  ...
  const handleNextClick = async () => {
    if (hasLetter && hasNumber && isLongEnough) {
      ...
      try {
        const response = await fetch('http://localhost:8080/sign-up/password', {
          method: 'PATCH',
          headers: {
            'Content-Type': 'application/json',
          },
          body: JSON.stringify({
            email: currentUpdatePasswordState.email,
            password: currentUpdatePasswordState.password,
          }),
        });

        if (response.ok) {
          router.push('/login/updatePassword/complete');
        } else{
          setShowError(true);
        }
      } catch (error) {
    ...
    <>
      ...
      <div style={styles.container}>
        <div style={{ width: '320px', margin: '0 auto' }}>
          ...
          <p style={styles.welcomeText}>새로운 비밀번호를 <br />입력 해주세요</p>
